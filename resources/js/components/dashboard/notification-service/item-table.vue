          ...
          <table class="table" id="printMe">
            <thead>
              <tr>
                ...
                <th scope="col">{{ $t("FACILITY") }}</th>
                <th scope="col">{{ $t("BRANCH_NUMBER") }}</th>
            ...
            <tbody v-else>
              <tr>
                <td class="no-result" colspan="11">{{ $t("No results") }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
import notificationServiceClient from "../../../http-clients/admin/notification-service-client";
...
export default {
  ...
  setup() {
    ...
    function downloadExcelFile() {
      notificationServiceClient.getAllItems().then((res) => {
        let data = res.data;
