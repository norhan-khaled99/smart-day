          ...
          <table class="table" id="printMe">
            <thead>
              <tr>
                ...
                <th style="width: 30%" scope="col">{{ $t("PRODUCT_NAME") }}</th>
                <th style="width: 20%" scope="col">{{ $t("CATEGORY") }}</th>
                <th style="width: 35%" scope="col">{{ $t("WITH_PHOTO") }}</th>
            ...
            <tbody v-if="items.length">
              <tr v-for="(item, index) in items" :key="item.id">
                ...
                <td>
                  <span class="normal" v-if="item.base_category">
                    {{
                      $i18n.locale == "ar"
                        ? item.base_category.name_ar
                        : item.base_category.name_en
                    }}
                  </span>
                ...
                <td>
                  <span class="chip">
                    {{ item.with_photo == 1 ?$t("WITH_PHOTO_VERIFICATION") : $t("WITHOUT_PHOTO") }}
                  </span>
                </td>
            ...
            <tbody v-else>
              <tr>
                <td class="no-result" colspan="6">{{ $t("No results") }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
import {inject,onMounted,provide,reactive,ref,toRefs,watch,} from "vue-demi";
...
export default {
  ...
  setup() {
    ...
    function togglePublished(id) {
      productClient.togglePublished(id).then((res) => {
    ...
    function getItems() {
      productClient
        .getItems(data.page, data.pageSize, data.base_category_id, data.withPhoto, data.text)
        .then((response) => {
    ...
    function downloadExcelFile() {
      productClient.getAllItems().then((res) => {
        let data = res.data;
